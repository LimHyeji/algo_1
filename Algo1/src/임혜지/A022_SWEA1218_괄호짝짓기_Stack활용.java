		...
		for(int test_case=1;test_case<=10;test_case++) {
			...
			for(int i=0;i<len;i++) {//문자 배열 처음부터 끝까지 돌면서
				if(strArr[i]=='{'||strArr[i]=='['||strArr[i]=='('||strArr[i]=='<') {//등장한 괄호가 왼쪽 괄호이면
					check.push(strArr[i]);//스택에 저장
				}
				else {//등장한 괄호가 오른쪽 괄호이면
				if(check.peek()=='{'&&strArr[i]=='}'||check.peek()=='['&&strArr[i]==']'||check.peek()=='('&&strArr[i]==')'||check.peek()=='<'&&strArr[i]=='>') {//짝이 맞으면
						check.pop();//저장했던 왼쪽 괄호 꺼냄
					}
